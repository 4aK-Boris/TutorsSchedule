...
fun UpdateContactScreen(
    ...
    viewModel: UpdateContactScreenViewModel = koinViewModel()
) {
    ...
    Scaffold(
        ...
        topBar = {
            TitleWithSaveIcon(
                title = stringResource(id = R.string.update_contact_screen_title),
                back = { viewModel.back(studentsNavigationListener) },
                save = { viewModel.updateContact(studentsNavigationListener, studentId, contactId) }
            )
        }
    ) { paddingValues ->
